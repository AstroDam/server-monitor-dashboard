...
async function evaluateAlertRules(serverId, hostname, metrics) {
    try {
        ...
        for (const rule of rulesResult.rows) {
            ...
            const openAlert = openAlertResult.rows[0];

            if (triggered && !openAlert) {
            ...
            if (triggered && openAlert) {
                const cooldownSeconds =
                    Number(rule.cooldown_seconds || 300);

                const lastNotificationAt =
                    openAlert.last_notification_at
                        ? new Date(openAlert.last_notification_at)
                        : null;

                const canNotifyAgain =
                    !lastNotificationAt ||
                    (
                        Date.now() - lastNotificationAt.getTime()
                ...
                if (canNotifyAgain) {
                    ...
                    await sendTelegramAlert({
                        title: `Alerta ainda ativo: ${rule.name}`,
                        server: hostname,
                        metric: rule.metric_name,
                        value: `${metric.value}${metric.unit || ''}`,
                        severity: rule.severity || 'warning'
...
router.delete('/servers/:id', authMiddleware, adminOnly, async (req, res) => {
    try {
        ...
        await pool.query(
            `DELETE FROM alert WHERE server_id = $1`,
            [id]
        );

        await pool.query(
            `DELETE FROM metric_sample WHERE server_id = $1`,
            [id]
        );

        await pool.query(
            `DELETE FROM server_heartbeat WHERE server_id = $1`,
            [id]
        );

        await pool.query(
            `DELETE FROM alert_rule WHERE server_id = $1`,
            [id]
        );
