...
function updateMetricModal(type) {
    ...
    const values = getMetricValues(type);

    const stats = calculateMetricStats(values);

    metricModalTitle.innerText = config.title;
    metricModalCurrent.innerText = `${current.toFixed(2)}%`;
    metricAverage.innerText = `${stats.average.toFixed(2)}%`;
    metricPeak.innerText = `${stats.peak.toFixed(2)}%`;
    metricMin.innerText = `${stats.min.toFixed(2)}%`;
...
async function updateUserRole(id) {
    try {
        ...
        if (!response.ok) {
            alert(data.error || 'Erro ao alterar perfil');
            return;
        }
        ...
        console.error('Erro alterar perfil:', error);
    }
}
...
async function deleteUser(id) {
    ...
    try {
        ...
        console.error('Erro remover usuário:', error);
    }
}
