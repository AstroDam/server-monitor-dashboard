...
const serversGrid =
    document.getElementById('servers-grid');
...
function renderMetricDetailChart(type) {
    ...
    if (type === 'disk') {
        ...
        labels =
            lastHistoryData.labels || [];

        values =
            lastHistoryData[config.key] || [];
...
if (dashboardServerFilter) {
    ...
    dashboardServerFilter.addEventListener(
        'change',
        () => {
...
if (alertServerFilter) {
    ...
    alertServerFilter.addEventListener(
        'change',
        () => {
